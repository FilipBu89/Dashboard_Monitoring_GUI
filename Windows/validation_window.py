...
COMPANY_NAME_COL, DESCRIPTION_TYPE_COL = (0,7)

class ValidationWindow(tkinter.Toplevel):
    ...
    """
    def __init__(self,main_window,parent,identifier: str,configuration: str, 
                connection: sqlite3.Connection,description_ignore: List[str]) -> None:
        super().__init__(parent)
        self.parent = parent
        self.description_ignore = description_ignore
        ...
        self.title(f"Batch Item | Identifier: {identifier} | Config: {configuration}")
        ...
        self.list_box.config(columns=('CompanyName','ConnectionName','ConfigurationName','Filename','Identifier',
                                    'Worker','Type','Description','DescriptionBusiness','Date','Data Owner'))
        ...
        escalated_button = tkinter.Button(frame,text="Escalated",command=self.__move_to_escalate)
        ...
        escalated_button.grid(row=0,column=1,sticky=("n","e","w"),pady=59, padx=3)
        junked_button.grid(row=0,column=1,sticky=("n","e","w"),pady=89, padx=3)
        ignored_button.grid(row=0,column=1,sticky=("n","e","w"),pady=129, padx=3)
    ...
    def __move_to_fix(self,*ignore):
        ...
        for row in rows:
            description = self.list_box.get(row)
            description_types.add(description[0][DESCRIPTION_TYPE_COL])
        query = app_queries.validate_identifier_items(self.identifier,description_types,1)
    ...
    def __move_to_escalate(self,*ignore):
        ...
        for row in rows:
            description = self.list_box.get(row)
            description_types.add(description[0][DESCRIPTION_TYPE_COL])
        query = app_queries.validate_identifier_items(self.identifier,description_types,2,comment)
        commands.update_insert_data(self.connection,query=query)
        self.show_list()
    ...
    def __move_to_junk(self,*ignore):
        ...
        for row in rows:
            description = self.list_box.get(row)
            description_types.add(description[0][DESCRIPTION_TYPE_COL])
        query = app_queries.validate_identifier_items(self.identifier,description_types,0)
        commands.update_insert_data(self.connection,query=query)
        self.show_list()
    ...
    def __move_to_ignore(self,*ignore):
        ...
        for row in rows:
            ...
            description_types.add(item[0][DESCRIPTION_TYPE_COL])
            company_names.add(item[0][COMPANY_NAME_COL])
        query = app_queries.validate_identifier_items_pernament(self.identifier,self.configuration_name,
                                                        company_names,description_types,3,comment)
        commands.update_insert_data(self.connection,query=query)
        self.show_list()

    def show_list(self,*ignore):
        query_result = commands.select_data(self.connection,
                                    app_queries.get_identifier_items(self.identifier,self.description_ignore))
